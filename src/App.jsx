  ...
  return (
    <header className="flex items-center justify-between gap-4 p-4 border-b bg-gradient-to-r from-pink-50 to-white">
      <div className="flex items-center gap-3">
        <div className="rounded-full w-12 h-12 bg-gradient-to-br from-pink-400 to-pink-600 flex items-center justify-center text-white font-bold text-lg">Ø£Ù…</div>
        <div>
          <h1 className="text-lg font-bold">MOM â€” Ù…Ø°ÙƒÙ‘Ø±Ø© Ø£Ù…ÙŠ Ø§Ù„Ø°ÙƒÙŠØ©</h1>
          <p className="text-xs text-gray-500">Ù…ÙƒØ§Ù† ÙˆØ§Ø­Ø¯ ÙŠÙ†Ø¸Ù… ÙŠÙˆÙ…Ù‡Ø§ ÙˆÙŠØ®Ù„ÙŠÙƒ Ù‚Ø±ÙŠØ¨</p>
        </div>
      </div>
  ...
  return (
    <section className="p-4">
      <div className="flex items-center justify-between mb-2">
        <h2 className="text-lg font-semibold">Ø§Ù„Ø£Ø¯ÙˆÙŠØ©</h2>
        <button onClick={() => setOpenNew(true)} className="px-3 py-1 bg-pink-600 text-white rounded">+ Add</button>
      </div>

      <div className="grid gap-3">
        {meds.length === 0 && <p className="text-sm text-gray-500">No medicines yet â€” add one for daily reminders.</p>}
        {meds.map((m) => (
          <article key={m.id} className="bg-white rounded-xl p-3 shadow-sm flex items-center justify-between">
            <div>
              <div className="font-semibold">{m.name}</div>
              <div className="text-xs text-gray-500">Dose: {m.dose || '-'} â€¢ Time: {niceDate(m.time)}</div>
            </div>
...
function Appointments({ apps, onAdd, onUpdate, onRemove }) {
  ...
  return (
    <section className="p-4">
      <div className="flex items-center justify-between mb-2">
        <h2 className="text-lg font-semibold">Ù…ÙˆØ§Ø¹ÙŠØ¯</h2>
        ...
        {apps.map((a) => (
          <div key={a.id} className="bg-white rounded-xl p-3 shadow-sm flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-gray-500">{niceDate(a.datetime)} â€¢ {a.note || '-'}</div>
            </div>
...
function Shopping({ items, onAdd, onToggle, onRemove }) {
  ...
  return (
    <section className="p-4">
      <div className="flex items-center justify-between mb-2">
        <h2 className="text-lg font-semibold">Ù‚Ø§Ø¦Ù…Ø© Ø§Ù„ØªØ³ÙˆÙ‚</h2>
      </div>
      <div className="flex gap-2">
...
function Recipes({ recipes, onAdd, onRemove }) {
  ...
  return (
    <section className="p-4">
      <div className="flex items-center justify-between mb-2">
        <h2 className="text-lg font-semibold">ÙˆØµÙØ§Øª Ø£Ù…ÙŠ</h2>
        <button onClick={() => setOpen(true)} className="px-3 py-1 bg-white border rounded">+ Add</button>
      </div>
      <div className="grid gap-2">
        {recipes.length === 0 && <p className="text-sm text-gray-500">No recipes yet â€” add family favorites.</p>}
        {recipes.map(rc => (
...
function Photos({ photos, onAdd, onRemove }) {
  ...
  return (
    <section className="p-4">
      <div className="flex items-center justify-between mb-2">
        <h2 className="text-lg font-semibold">Ø°ÙƒØ±ÙŠØ§Øª ÙˆØµÙˆØ±</h2>
        <div>
  ...
  useEffect(() => {
    let interval = setInterval(() => {
      ...
      state.medications.forEach((m) => {
        ...
        if (diff < 40 * 1000) {
          askNotify && askNotify();
          notify(`ÙˆÙ‚Øª Ø§Ù„Ø¯ÙˆØ§Ø¡: ${m.name}`, { body: `Ø¬Ø±Ø¹Ø©: ${m.dose || '-'} Ø§Ù„Ø¢Ù†` });
        }
      });
      ...
      state.appointments.forEach((a) => {
        ...
        if (diff > 0 && diff < 10 * 60 * 1000) {
          askNotify && askNotify();
          notify(`Ù…ÙˆØ¹Ø¯ Ù‚Ø±ÙŠØ¨: ${a.title}`, { body: `ÙÙŠ ${Math.round(diff / 60000)} Ø¯Ù‚ÙŠÙ‚Ø©` });
        }
      });
    }, 30 * 1000);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-pink-50 to-white text-gray-800">
      <div className="max-w-5xl mx-auto">
        ...
        <footer className="p-4 text-center text-xs text-gray-500">Made with love â€” a simple helper to make everyday easier.</footer>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => {
              ...
              alert('Copied JSON to clipboard â€” paste it into a message to share.');
